...
画布 JSON 不直接长期保存大体积 base64 图片或视频。图片节点和视频节点只保存展示 URL、`storageKey` 和元信息，真实 Blob 通过 `storageKey` 读取。
...
1. 删除节点、清空画布或删除画布时，会触发 `cleanupImages`。
2. `cleanupImages` 会收集当前仍被画布项目和额外传入数据引用的所有 `storageKey`。
