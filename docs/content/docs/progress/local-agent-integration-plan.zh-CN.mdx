---
title: 本地 Agent 接入规划
description: 已完成规划：最初通过本地 Canvas Agent、MCP 与侧边栏助手连接 Codex / Claude Code，现已收敛进前端应用
---

# 本地 Agent 接入规划

> 说明：本规划已完成并关闭。它最初的形态是「本机 `canvas-mcp` + Codex / Claude Code 侧边栏」，最终实现收敛为：画布 MCP 融合进前端应用，前端服务在同一端口 3000 上同时提供浏览器桥接与 `/mcp`，不再有独立包、Codex / Claude 对话、Skill、审批、附件或 token。以下内容仅保留为早期设计记录。
