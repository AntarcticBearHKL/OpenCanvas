...
- 颜色一律走 token：画布用 `canvasThemes` / `theme.*`，Tailwind 用 `text-foreground`、`text-muted-foreground`、`bg-brand`、`bg-brand-soft`、`border-brand`、`text-success`、`text-warning`、`text-danger`、`text-info` 及各自的 `*-soft`；交互反馈统一用 `hover:bg-hover`（`--hover` 由主题提供：浅色是品牌暖调、深色是提亮），不要再写 `hover:bg-black/5 dark:hover:bg-white/10`。
- 只有「压在任意图片上」的内容物允许写死黑白：视频 / 图片的 letterbox 底色、裁剪与蒙版的可视手柄与画笔光标、棋盘格、图片预览的深色遮罩。
- 选中 / 激活态全站只有一种画法：`theme.node.accentSoft` 作底 + `theme.node.accent` 作文字（需要描边时用 `inset 0 0 0 1px theme.node.accent`）——面板页签、工作台页签、工具按钮、分段控件共用同一个样式；未选中一律 `theme.node.muted`，主操作用品牌实底（AntD primary）。不要再新增 `toolbar.accentBg/accentText` 这类同义 token，也不要让 AntD 的链接色接管顶部导航——导航这类 `<a>` 元素必须显式写 `!text-foreground` / `!text-muted-foreground` 覆盖。
...
- 画布内的操作按钮（如面板里的「添加」「导出」「选择」等）默认用扁平无底色样式：透明背景、仅 `hover:bg-hover` 轻微反馈，靠图标+文字表达，不要用 `theme.toolbar.activeBg`（`#e7e5df`/`#3a3631`）或 `theme.node.fill` 之类的灰色作为按钮填充底色。灰色 `activeBg` 只允许用于「选中态」等需要表达状态的高亮，不要当普通装饰底色。
...
- 当前画布项目、文本创作器的作品与生成记录都保存在浏览器本地，不要在文档中误写成已支持云同步（素材库已整体移除，不再有「我的素材」）。
- 智能画板节点的 width/height 既是画布上的卡片尺寸、也是画板文档的坐标系（图层、选区、裁剪、旋转、栅格导出都按它换算）。不要在画布上自由缩放智能画板节点来改大小：要改文档尺寸必须走「图像大小 / 画布大小」（会同步重采样图层）或改比例（会重新推导尺寸并居中），否则已有图层会留在旧坐标系里、看起来只剩左上角一块。
