    ...
    const pluginAi = useMemo<CanvasPluginAi>(() => {
        ...
        return {
            generateImage: async (prompt, options) => {
                ...
                const { images: items } = references.length ? await requestEdit(config, prompt, references, { signal: options?.signal }) : await requestGeneration(config, prompt, { signal: options?.signal });
