...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, showMode = true, showSize = true, className = "w-[320px] space-y-4 rounded-none px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    const resolutionColumns = capability.resolutions.length === 1 ? "grid-cols-1" : capability.resolutions.length === 2 ? "grid-cols-2" : "grid-cols-4";
        ...
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title={t("settingsPanels.video.quality")} color={theme.node.muted}>
                    <div className={`grid gap-2.5 ${resolutionColumns}`}>
                ...
                {capability.supportsAudio ? (
                    <SettingGroup title={t("settingsPanels.video.audio")} color={theme.node.muted}>
                        <div className="grid grid-cols-2 gap-2.5">
                            <OptionPill selected={generateAudio} theme={theme} onClick={() => onConfigChange("videoGenerateAudio", "true")}>
                                {t("settingsPanels.video.audioOn")}
                            </OptionPill>
                            <OptionPill selected={!generateAudio} theme={theme} onClick={() => onConfigChange("videoGenerateAudio", "false")}>
                                {t("settingsPanels.video.audioOff")}
                            </OptionPill>
                        </div>
                    </SettingGroup>
                ) : null}
...
export function videoResolutionLabel(value: string, videoModel?: string) {
    const resolution = supportedVideoResolution(value, videoModelCapability(videoModel)) ?? parseVideoResolution(value);
