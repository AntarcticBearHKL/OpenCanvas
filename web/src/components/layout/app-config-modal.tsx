...
import { ConfigModels } from "@/components/layout/config-models";
import { ConfigProviders } from "@/components/layout/config-providers";
...
export function AppConfigPanel() {
    ...
    return (
        <Tabs
            ...
            classNames={{ header: "app-config-tabs-header w-52 shrink-0 px-2.5 py-3", item: "[&_.ant-tabs-tab-btn]:flex [&_.ant-tabs-tab-btn]:items-center [&_.ant-tabs-tab-btn]:text-sm [&.ant-tabs-tab-active_.ant-tabs-tab-btn]:font-semibold" }}
            styles={{ item: { padding: "5px 10px", borderRadius: 2, marginTop: 2 }, content: { padding: "20px 24px" } }}
            items={[
                {
                    key: "channels",
                    label: t("config.tabs.providers"),
...
export function AppConfigModal() {
    ...
    return (
        <Modal
            ...
            footer={null}
            classNames={{ container: "glass-raised !bg-[var(--glass-strong)]" }}
            styles={{
                container: { padding: 0, overflow: "hidden", borderRadius: 0, border: `1px solid ${token.colorBorder}` },
