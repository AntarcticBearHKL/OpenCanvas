        ...
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={860} centered destroyOnHidden classNames={{ container: "glass-raised" }} styles={{ container: { background: "var(--glass-strong)" } }}>
            <div className="space-y-5">
                ...
                <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_320px]">
                    <div className="min-w-0 rounded-none border p-4" style={{ borderColor: theme.node.stroke }}>
                        <div className="relative inline-block">
                            <img src={dataUrl} alt="" className="block max-h-[340px] w-auto max-w-full cursor-crosshair rounded-[2px]" draggable={false} onPointerDown={addPoint} onContextMenu={(event) => event.preventDefault()} />
                            {meta
                        ...
                        <div className="mt-3 text-sm" style={{ color: theme.node.muted }}>{t("canvas.segment.hint")}</div>
                    </div>
                    <div className="min-w-0 space-y-4">
                        ...
                        <div className="text-xs" style={{ color: theme.node.muted }}>{points.length ? `${points.length}` : t("canvas.segment.empty")}</div>
                        ...
                        {result ? (
                            <div className="space-y-2 rounded-none border p-3" style={{ borderColor: theme.node.stroke }}>
                                <div className="text-sm font-medium" style={{ color: theme.node.label }}>{t("canvas.segment.result")}</div>
                                <div className="text-sm">{t("canvas.segment.score", { score: result.score.toFixed(3) })}</div>
                                <div
                                    className="grid place-items-center rounded-[2px] p-2"
