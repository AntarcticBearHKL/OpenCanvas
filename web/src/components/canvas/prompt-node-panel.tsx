                ...
                <div className="mb-2 flex max-w-full shrink-0 flex-wrap items-center gap-1" title={t("canvas.promptNode.tagHint")} onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
                    {tags.map((tag) => (
                        <button
                            key={tag}
                            type="button"
                            className="inline-flex h-7 shrink-0 cursor-pointer items-center rounded-md px-2 text-sm transition hover:bg-hover"
        ...
        >
            {images.map((reference) => (
                <div
                    key={reference.id}
                    draggable
                    className="canvas-prompt-ref-chip flex h-7 max-w-32 cursor-grab items-center gap-1 overflow-hidden rounded-md border px-1 text-sm leading-none"
                ...
                >
                    {reference.previewUrl ? <img src={reference.previewUrl} alt="" draggable={false} className="size-5 shrink-0 rounded-md object-cover" /> : null}
