...
function PickerCard({ title, kind, cover, onClick }: { title: string; kind: string; cover: string; onClick: () => void }) {
    ...
    return (
        ...
        >
            ...
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-background/0 text-sm font-medium text-foreground opacity-0 transition group-hover:bg-background/55 group-hover:opacity-100">{t("canvas.assetPicker.insert")}</div>
        </button>
    );
}
