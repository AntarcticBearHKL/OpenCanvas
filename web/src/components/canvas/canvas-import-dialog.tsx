...
export function CanvasImportDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    return (
        <Modal title={t("canvas.importDialog.title")} open={open} onCancel={cancel} footer={null} centered>
            <button
                type="button"
                className="flex h-44 w-full cursor-pointer flex-col items-center justify-center gap-2 rounded-md border border-dashed px-4 text-center text-sm transition"
