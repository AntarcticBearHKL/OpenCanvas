...
export function AudioProjectNodeContent({ node, theme }: { node: CanvasNodeData; theme: CanvasTheme }) {
    ...
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-2 px-5 text-center" style={{ color: theme.node.placeholder }}>
            <AudioWaveform className="size-7 opacity-35" />
            <span className="text-xs">{t("canvas.audioStudio.summary", { tracks: tracks.length, clips: clips.length })}</span>
            <span className="text-[11px] tabular-nums opacity-70">{formatAudioTime(audioProjectDuration(clips))}</span>
        </div>
    );
}
